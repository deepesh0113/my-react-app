...
function Dashboard() {
  ...
  const [distributionData, setDistributionData] = useState([]); // histogram
  const [summary, setSummary] = useState(null); // normalized summary {min,max,mean,n}
  ...
  const RED = "#ef4444";
  const AMBER = "#facc15";
  const LENSALERT = "#a855f7"; // violet for lens_covered_or_extremely_dark
  const FREEZEALERT = "#f97316"; // orange for camera_frozen
  ...
  const styles = {
    ...
    graphBox: {
      background:
        "radial-gradient(circle at top right, #020617, #020617 70%)",
      borderRadius: "18px",
      border: "1px solid #111827",
  ...
  const renderColoredDot = (props) => {
    ...
    if (alertType === "lens_covered_or_extremely_dark") {
      ...
      // 2) no alert string -> threshold colors
      if (value > parsedMax) fill = RED;
      else if (value >= parsedMin && value <= parsedMax) fill = GREEN;
      else fill = AMBER;
    }
  ...
  const applyDateFilter = (date, recordsSrc, perSecondSrc) => {
    ...
    if (!date) {
      ...
      computeDistribution(recordsSrc);
      return;
    }
    ...
    computeDistribution(filteredRecords);
  };
  ...
  const computeDistribution = (records) => {
    ...
    records.forEach((r) => {
      const binIndex = Math.min(
        Math.floor((r.count - minCount) / binSize),
        9
      );
      bins[binIndex]++;
    });
    ...
    const distribution = bins.map((count, index) => ({
      bin: index + 1,
      range: `${Math.round(minCount + index * binSize)}-${Math.round(
        minCount + (index + 1) * binSize
      )}`,
      frequency: count,
      // representative count (bin center)
  ...
  const applyMonthFilter = (monthKey, recordsSrc) => {
    if (!monthKey || !recordsSrc || !recordsSrc.length) {
      setMonthDailyData([]);
      return;
    }

    const monthRecords = recordsSrc.filter((r) =>
      r.date ? r.date.startsWith(monthKey) : false
    );
    const dayMap = {};
  ...
  const applyYearFilter = (yearKey, recordsSrc) => {
    if (!yearKey || !recordsSrc || !recordsSrc.length) {
      setYearMonthlyData([]);
      return;
    }

    const yearRecords = recordsSrc.filter((r) =>
      r.date ? r.date.startsWith(yearKey) : false
    );
    const monthMap = {};

    yearRecords.forEach((r) => {
      const mkey = r.date.slice(0, 7); // YYYY-MM
      const shortMonth = mkey.slice(5, 7); // 01, 02, ...
      if (!monthMap[mkey]) {
        monthMap[mkey] = {
          month: shortMonth,
          monthLabel: mkey,
      ...
      monthMap[mkey].total += r.count;
      monthMap[mkey].max = Math.max(monthMap[mkey].max, r.count);
      monthMap[mkey].n += 1;
    });
  ...
  const handleCSVUpload = async () => {
    ...
    try {
      const res = await fetch(
        "http://127.0.0.1:8000/dashboard/upload-csv",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (rawSummary) {
        const min =
          rawSummary.min_count ??
          rawSummary.mincount ??
          0;
        const max =
          rawSummary.max_count ??
          rawSummary.maxcount ??
          0;
        const mean =
          rawSummary.mean_count ??
          rawSummary.meancount ??
          0;
        const n =
          rawSummary.num_points ??
          rawSummary.numpoints ??
          0;
        normSummary = { min, max, mean, n };
      }
  ...
  const getPerSecondSafe = () =>
    thresholdsActive
      ? perSecondData.filter(
        (p) =>
          p.avg_count >= parsedMin && p.avg_count <= parsedMax
      )
      : [];
  ...
  const getMonthSafeDays = () =>
    thresholdsActive
      ? monthDailyData.filter(
        (d) =>
          d.avg_count >= parsedMin && d.avg_count <= parsedMax
      )
      : [];

  // Year-wise detail helpers
  ...
  const getYearSafeMonths = () =>
    thresholdsActive
      ? yearMonthlyData.filter(
        (m) =>
          m.avg_count >= parsedMin && m.avg_count <= parsedMax
      )
      : [];
      ...
      <div style={styles.content}>
        ...
        <div style={styles.theoryCard}>
          ...
          <p style={styles.theoryText}>
            VigilNet&apos;s historical dashboard turns{" "}
            <code>date,timestamp_ms,count,alert</code> logs into a visual
            narrative. Zoom into a single day or step back to see how whole
            months behave, spotting spikes, quiet days, and recurring patterns.
          </p>
          <ul style={styles.theoryList}>
            <li>
              <strong>Day-wise:</strong> inspect how counts evolve within a day
              (timestamp-wise, per-second, distribution)
            </li>
            <li>
            ...
            <li>
              <strong>Year-wise:</strong> compare months within a year for
              planning and capacity checks
            </li>
          </ul>
        </div>
            ...
            <div style={styles.historyCard}>
              ...
              {selectedHistory && (
                ...
                >
                  <div>
                    Min count:{" "}
                    {typeof selectedHistory.minCount === "number"
                      ? selectedHistory.minCount.toFixed(2)
                      : selectedHistory.minCount}
                  </div>
                  <div>
                    Max count:{" "}
                    {typeof selectedHistory.maxCount === "number"
          ...
          <div style={styles.rightCol}>
            ...
            <div style={styles.graphBox}>
              ...
              <div style={styles.graphSubtitle}>
                ...
                Legend additions:
                <span
                  style={{ color: LENSALERT, fontWeight: "bold" }}
                >
                  {" "}
                  ●
                </span>{" "}
                violet dot = lens covered or extremely dark
                <br />
                <span
                  style={{ color: FREEZEALERT, fontWeight: "bold" }}
                >
                  {" "}
                  ●
                </span>{" "}
                orange dot = camera frozen
              </div>
              <input
                type="file"
                accept=".csv"
              ...
              {summary && (
                <div style={styles.summaryRow}>
                  <span style={styles.summaryChip}>
                    Min count: {summary.min.toFixed(2)}
                  </span>
                  <span style={styles.summaryChip}>
                    Max count: {summary.max.toFixed(2)}
                  </span>
                  <span style={styles.summaryChip}>
                    Mean count: {summary.mean.toFixed(2)}
                  </span>
                  <span style={styles.summaryChip}>
                    Points: {summary.n}
                  </span>
            ...
            {viewMode === "day" ? (
              <>
                ...
                <div style={styles.graphBox}>
                  ...
                  <div style={styles.graphSubtitle}>
                    Share of frames that are below min, safe, above max, lens covered, or
                    camera frozen.
                  </div>
                  ...
                  {graphData.length > 0 && (
                    <div style={styles.summaryRow}>
                      {(() => {
                        ...
                        const pieData = [
                          { name: "Below min", value: below, color: AMBER },
                        ...
                        return (
                          <>
                            ...
                            <span style={styles.summaryChip}>
                              Lens covered: {lens} • Frozen: {frozen}
                            </span>

                            <div style={styles.graphArea}>
                              {pieData.length === 0 ? (
                                <div style={styles.graphPlaceholder}>
                                  No categorized frames for this day.
                                </div>
                              ) : (
                                <ResponsiveContainer width="100%" height="100%">
                                  <PieChart>
                  ...
                  {graphData.length === 0 && (
                    <div style={styles.graphArea}>
                      <div style={styles.graphPlaceholder}>
                        Upload a CSV and select a date to see distribution.
                      </div>
                    </div>
                  )}
                </div>
              ...
              <>
                {/* MONTH-WISE GRAPH 1 – avg per day */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                    Month overview: average crowd per day
                  </div>
                  <div style={styles.graphSubtitle}>
                    Each point = one day. Useful for spotting consistently busy
                    or calm days in the selected month.
                  </div>

                  <button
                    style={styles.detailBtn}
                    onClick={() =>
                      setShowMonthDetails((prev) => !prev)
                    }
                  >
                      ...
                      {getMonthAlertDays().length === 0 ? (
                        ...
                        getMonthAlertDays().map((d, idx) => (
                          <div key={`md-alert-${idx}`}>
                            day = {d.day}, avg = {d.avg_count.toFixed(2)},
                            max = {d.max_count.toFixed(2)}, total ={" "}
                ...
                {/* MONTH-WISE GRAPH 2 – max per day */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                  ...
                  <button
                    style={styles.detailBtn}
                    onClick={() =>
                      setShowMonthDetails2((prev) => !prev)
                    }
                  >
                    {showMonthDetails2
                      ? "Hide details"
                ...
                {/* MONTH-WISE GRAPH 3 – total per day */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                  ...
                  <button
                    style={styles.detailBtn}
                    onClick={() =>
                      setShowMonthDetails3((prev) => !prev)
                    }
                  >
                    {showMonthDetails3
                      ? "Hide details"
                      ...
                      {getMonthAlertDays().length === 0 ? (
                        ...
                        getMonthAlertDays().map((d, idx) => (
                          <div key={`md3-alert-${idx}`}>
                            day = {d.day}, total ={" "}
                            {d.total_count.toFixed(2)}, max ={" "}
                            {d.max_count.toFixed(2)}, avg ={" "}
                            {d.avg_count.toFixed(2)}
                          </div>
                        ))
                      )}
              ...
              <>
                {/* YEAR-WISE GRAPH 1 – avg per month */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                    Year overview: average crowd per month
                  </div>
                  <div style={styles.graphSubtitle}>
                    Each point = one month. Helps you compare how busy months
                    are within the selected year.
                  </div>

                  <button
                    style={styles.detailBtn}
                    onClick={() =>
                      setShowYearDetails((prev) => !prev)
                    }
                  >
                  ...
                  <div style={styles.graphArea}>
                    {yearMonthlyData.length === 0 ? (
                      <div style={styles.graphPlaceholder}>
                        Upload a CSV and pick a year to see month-wise
                        averages.
                      </div>
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
                      ...
                      {getYearAlertMonths().length === 0 ? (
                        ...
                        getYearAlertMonths().map((m, idx) => (
                          <div key={`ym-alert-${idx}`}>
                            month = {m.monthLabel}, avg ={" "}
                            {m.avg_count.toFixed(2)}, max ={" "}
                            {m.max_count.toFixed(2)}, total ={" "}
                ...
                {/* YEAR-WISE GRAPH 2 – peak per month */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                    Year overview: peak crowd per month
                  </div>
                  <div style={styles.graphSubtitle}>
                    Shows the highest count observed in each month of the
                    selected year.
                  </div>
                  ...
                  <div style={styles.graphArea}>
                    {yearMonthlyData.length === 0 ? (
                      <div style={styles.graphPlaceholder}>
                        Upload a CSV and pick a year to see monthly max
                        crowd.
                      </div>
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
                ...
                {/* YEAR-WISE GRAPH 3 – total per month */}
                <div style={styles.graphBox}>
                  <div style={styles.graphTitle}>
                    Year overview: total crowd signal per month
                  </div>
                  <div style={styles.graphSubtitle}>
                    Sum of all counts for each month – useful for annual
                    planning and capacity checks.
                  </div>
                  ...
                  <div style={styles.graphArea}>
                    {yearMonthlyData.length === 0 ? (
                      <div style={styles.graphPlaceholder}>
                        Upload a CSV and pick a year to see total signal per
                        month.
                      </div>
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
